...
export default function OutboundPage() {
  ...
  useEffect(() => {
    fetchOutboundData();
  }, [currentPage, pageSize, dateRange.start, dateRange.end, searchKeyword, selectedCategory, selectedOutboundType]);
  ...
  const handleExcelDownload = async () => {
    try {
      const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
      const response = await fetch(`${baseUrl}/api/v1/inventory-out/excel`, {
        method: 'GET',
        headers: {
