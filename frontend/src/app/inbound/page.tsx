...
export default function InboundPage() {
  ...
  const [pageSize, setPageSize] = useState(20); // 10에서 20으로 변경
  ...
  const categories = useMemo(() => {
    ...
    const uniqueCategories = new Set(
      inboundData.content.map((item) => item.categoryName)
    );
  ...
  const fetchInboundData = async () => {
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/v1/inventory-in?${queryParams}`,
        {
          method: "GET",
          headers: {
      ...
      if (data.content) {
        ...
        if (searchKeyword.trim()) {
          ...
          filtered = filtered.filter(
            (item) => item.itemName.toLowerCase().includes(keyword) // itemName만으로 필터링
          );
        }
        ...
        if (dateRange.start && dateRange.end) {
          filtered = filtered.filter((item) => {
            ...
            end.setHours(23, 59, 59); // 종료일의 끝시간으로 설정
  ...
  useEffect(() => {
    if (inboundData) {
      // 프론트엔드에서 필터링 수행
      let filtered = inboundData.content;
  ...
  }, [inboundData, searchKeyword, dateRange, selectedCategory]);
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200 hover:border-[#0047AB] transition-all duration-200">
              <div className="flex flex-col">
                ...
                <span className="text-3xl font-semibold text-gray-900 mb-1">
                  {calculateStats(filteredContent).total}
                </span>
            ...
            <table className="w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[60px] whitespace-nowrap">
                    이미지
                  </th>
                </tr>
              </thead>
          ...
          <div className="mt-6 flex justify-center">
            <nav className="flex items-center gap-2">
              ...
              <button
                onClick={() =>
                  setCurrentPage(
                    Math.min(inboundData.totalPages, currentPage + 1)
                  )
                }
